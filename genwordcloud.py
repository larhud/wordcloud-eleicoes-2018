...
def generate(filename):
    excecoes = []
    for palavra in open('excecoes.txt').read().lower().split(','):
        excecoes.append(palavra.strip())
    for palavra in open('contexto.txt').read().lower().split(','):
        excecoes.append(palavra.strip())

    palavras = re.findall(r'\w+', open(filename).read().lower())
    ...
    print('Cloud %s gerada' % image_filename)
